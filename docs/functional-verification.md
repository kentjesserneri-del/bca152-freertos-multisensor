...
This is the record of testing the system in Wokwi to confirm it actually behaves correctly, not just that it builds. I went through each feature and wrote down what I actually saw on screen or in the terminal.

**About the I2C warnings (resolved):** earlier runs printed recurring lines like `W (8533) i2c.common: GPIO 18 is not usable, maybe conflict with others`. I later traced this by comparing exact log timestamps against the actual code and found the warnings only ever showed up during bursts of many I2C transactions sent back to back with no delay between them, specifically `display_init()`'s command sequence and `display_clear()`'s full-screen redraw, never during a normal single-value sensor update. Adding a 1-tick `vTaskDelay` between I2C transactions in `display.cpp` eliminated the warnings completely, confirmed by comparing identical boot and INACTIVE-transition runs before and after the change. See the lab report's Limitations section for the full writeup. The results below were recorded before this fix was found, so some of the raw terminal output quoted still shows the old warnings; that's expected and doesn't affect what each test was actually checking.
